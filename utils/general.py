...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference    
        ...
        if nc == 1:
            ...
            x[:, 5:] *= x[:, 4:5].clone()  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            ...
            for i,f in enumerate(conf.view(-1) > conf_thres):
                if f.item()==False:
                    true_index.append(i)
        ...
        for c in range(len(i)):
            count=0
            for b in range(len(true_index)):
                if i[c].item()>=true_index[b]:
                    count+=1
            need_index.append(i[c].item()+count)   

        
        output[xi] = x[i]
        print(output[xi].shape)
        for i,f in enumerate(output[xi]):
                output2.append((need[need_index[i]],f[5]))
