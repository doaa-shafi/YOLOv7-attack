...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i,f in enumerate(output2[0]):
            if int(f[2].item())-int(f[0].item())<50 or int(f[3].item())-int(f[1].item())<50:
                mask.append((0,0))
            else:
                mask.append((int(f[0].item()),int(f[1].item()),int(f[2].item()),int(f[3].item())))  
        # mask=generate_mask(masks)
...
if __name__ == '__main__':
    ...
    for i in range(2):
        for a,b in enumerate(need):
            if len(mask[a])!=2  :
                ...
                if i==1:
                    ...
                    factor=10**factor
                if i<=2:
                    factor1=int((b[0][top2in[0]]).item()/(b[0][top2in[2]]).item())
                    factor1=math.log(factor1,10)
                    factor1 = round(factor1, 0)
                    factor1=10**factor1
                factor2=1
                if i<=2:
                    factor2=int((b[0][top2in[1]]).item()/(b[0][top2in[2]]).item())
                    factor2=math.log(factor2,10)
                    factor2 = round(factor2, 0)
                    factor2=10**factor2
                print(factor)
                # r=(factor2+factor1)*data_grad3-1*data_grad2-data_grad1
                # mode=3
        ...
        new = transform(vis)
        new.save(last+'-'+str(mode)+'-'+str(i+1)+'.jpg')
        opt.source=last+'-'+str(mode)+'-'+str(i+1)+'.jpg'
        detect()
